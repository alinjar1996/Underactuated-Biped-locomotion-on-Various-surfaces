...
n = 10;% no of half cycles
% n= 4 to best match with Ono result
t0 = 0;
...
[t1,q1] = ode45(@odefun1,tspan,q0,options1);
  ...
  for jj =1:length(t1)
      ...
      %Thip1(jj,:) = abs((k*q1(jj,3)+kd*q1(jj,6))*q1(jj,5)*D(jj));
...
options2 = odeset('Events',@groundimpact_flat);%,'MaxStep',2.5000e-05);
...
%     temp =[2*pi-q0(1) q0(2) -q0(3) q0(6) q0(5) q0(4)];
    ...
    figure()
   hold on
   graph1=plot(qq(:,3)*180/pi,qq(:,6)*180*1/pi);
   % graph2 = plot(qq(:,1)*180/pi-180,qq(:,4)*180/pi);
     graph2 = plot(qq(:,1)*180/pi-180,qq(:,4)*180*1/pi);
  %  title('Half cycle=',num2str(j))
