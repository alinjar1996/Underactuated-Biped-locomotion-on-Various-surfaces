...
set(gca,'fontsize',38, 'fontname', 'Euclid')
...
%swing_leg_clearance
global savex5
%swlegclearance = zeros(1,length(t));

for i =1:length(savex5)
   % if Q1(i)==0
  % ydash(i)=0;
  %  else
   ydash(i) = tand(2)*savex5(i);
   swlegclearance1(i)= savey5(i)-ydash(i);  %Straight Vertical distance from foot to ground 
   swlegclearance2(i)= savey5(i)-Q1(i); % vertical distance between foot and base; only required for first half cycle
...
plot(t,swlegclearance.*1000,  'linewidth', 3)
xlabel('Time (s)')
ylabel('Clearance (mm)')
set(gca,'fontsize',36, 'fontname', "Euclid")
