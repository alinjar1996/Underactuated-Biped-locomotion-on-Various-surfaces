function [] = Display1_2degincline(q,t,P1,Q1)
...
x1=P1(i); % foot (fixed base)
%   y1=zeros(length(x1),1);
%   m=tand(-4);
%   y1=-m.*x1;
 y1=Q1(i); % foot (fixed base)
th1=X(1);
th2=X(2);
th3=X(3);
x2=x1+l(1)*sin(X(1))/2;
...
% axis('equal')
m = tand(2);
 xx=linspace(0,-5,10000);%for inclined plane
...
%    set(animation,'LineWidth',2);
     ...
     hold off
   L1=line([savex1(j) savex2(j)],[savey1(j) savey2(j)],'LineWidth',2);
   L2=line([savex2(j) savex3(j)],[savey2(j) savey3(j)],'LineWidth',2);
   L3=line([savex3(j) savex4(j)],[savey3(j) savey4(j)],'LineWidth',2);
   L4=line([savex4(j) savex5(j)],[savey4(j) savey5(j)],'LineWidth',2);
    P=[saveX1(j) saveY1(j)];
    Q=[saveX2(j) saveY2(j)];
    R=[saveX3(j) saveY3(j)];
%     xlabel('Time','fontweight','bold','fontsize',16);
    xlabel('$x$','fontweight','bold','fontsize',20, Interpreter='latex');
    ylabel('$y$','fontweight','bold','fontsize',20, Interpreter='latex');
